...
def connect(dbname):
    ...
    try:
        ...
        try:
            ...
            db = pg.connect(database=dbname, host="/var/run/postgresql/")
        except:
            log.error("Cannot connect to database {0}. Is postgresql running?".format(dbname))
            sys.exit()
    return db
...
def deleteTable(dbname, schemaname, tablename):
    """Delete database if it exists."""
    if tableExists(dbname, schemaname, tablename):
        db = connect(dbname)
        cur = db.cursor()
        cur.execute("drop table {0}.{1}".format(schemaname, tablename))
        db.commit()
        cur.close()
        db.close()
...
def setAttribute(dbname, table, column, value):
    """Set attribute column in raster data tables, including resampled tables."""
    sname, tname = table.split(".")
    db = connect(dbname)
    cur = db.cursor()
    cur.execute("select distinct(resolution) from vic.soils")
    if bool(cur.rowcount):
        resolutions = [r[0] for r in cur.fetchall()] + [0.0]
        for res in resolutions:
            ext = "_{0}".format(int(1.0 / res)) if res > 0 else ""
            if not columnExists(dbname, sname, "{0}{1}".format(tname, ext), column):
                cur.execute("alter table {0}.{1}{2} add column {3} int".format(sname, tname, ext, column))
                db.commit()
            sql = "update {0}.{1}{2} set {3} = {4} where {3} is null".format(sname, tname, ext, column, value)
            cur.execute(sql)
    db.commit()
    cur.close()
    db.close()
...
def writeGeotif(lat, lon, res, data, filename=None, epsg=4326):
...
def getResampledTables(dbname, options, res):
    ...
    for v in ['precip', 'tmax', 'tmin', 'wind']:
        tname = options['vic'][v]
        rtables[v] = "{0}_{1}".format(tname, int(1.0 / res))
    cur.close()
    db.close()
    return rtables
...
def ingest(dbname, filename, dt, stname, resample=True, overwrite=True):
    ...
    cmd = "raster2pgsql -d -s 4326 {0} {2} | psql -d {1}".format(filename, dbname, temptable)
