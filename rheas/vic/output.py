...
OUTVAR          OUT_PAR         %.4f    *       *
OUTVAR          OUT_PET_SHORT   %.6f    *       *
OUTVAR          OUT_PET_TALL    %.6f    *       *
OUTVAR          OUT_PET_NATVEG  %.6f    *       *
"""
...
# Flux files added for routing module read in runff & Baseflow 
#   VIKALP MISHRA
#   AUG 2021
#   SERVIR SCO

flxTemplate = """OUTFILE        fluxes     5
OUTVAR          OUT_PREC        %.4f    *       *
OUTVAR          OUT_EVAP        %.4f    *       *
OUTVAR          OUT_RUNOFF      %.4f    *       *
OUTVAR          OUT_BASEFLOW    %.4f    *       *
OUTVAR          OUT_SOIL_MOIST  %.4f    *       *
"""

def template(varlist):
    """Returns string of VIC output template."""
    out = "N_OUTFILES\t{0}\n".format(len(varlist)+1)
    out += "\n"
    out += flxTemplate   #hardcoded flux output files 
    if "eb" in varlist:
        out += ebTemplate
    if "csp" in varlist:
        out += cspTemplate
