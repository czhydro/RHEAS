...
class VIC:
    ...
    def __init__(self, path, dbname, resolution, startyear, startmonth, startday,
                 endyear, endmonth, endday, basin, name="", savestate="", nlayer=3):
    ...
    def writeForcings(self, prec, tmax, tmin, wind, lai=None):
        ...
        tmin = met2df(tmin)
        wind = met2df(wind)
        print(len(prec),len(tmax),len(tmin),len(wind))
        #sys.exit()
    ...
    def writeToDB(self, data, dates, tablename, initialize, ensemble=False, skipsave=0):
        ...
        with tempfile.NamedTemporaryFile('a') as fout:
            subprocess.call(["raster2pgsql", "-s", "4326", "-F", "-d"] + tiffiles[:min(365, len(tiffiles))] + ["temp"], stdout=fout)
            for i in range(1, int(len(tiffiles) / 365)+1):
                fnames = tiffiles[i*365:min((i+1)*365, len(tiffiles))]
                fout.flush()
                subprocess.call(["raster2pgsql", "-s", "4326", "-F", "-a"] + fnames + ["temp"], stdout=fout)
            cmd = ["psql", "-d", self.dbname, "-f", fout.name]
            proc = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
            for line in iter(proc.stdout.readline, b''):
                log.debug(line.strip())
    ...
    def analysis(self,rout_init):
        from rheas import result_analysis
        
        result_analysis.res_analysis(self,vic=True)
        if rout_init:
            result_analysis.res_analysis(self,rout=True)
