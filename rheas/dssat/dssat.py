...
class DSSAT(object):
    ...
    def __init__(self, dbname, name, resolution, startyear, startmonth, startday,
        ...
        self.dbpath = "./output/{0}".format(name)
        # self.path = 'debug'; 
    ...
    def readVICOutputFromDB(self, gid, depths, planting, harvest):
        ...
        sql = "select max(fdate) from {0}.{1} where {2} and ensemble=0".format(self.name, varnames[0], date_sql)
        cur.execute(sql)
        ...
        if (nowcast_date + timedelta(1) <= enddate):                
            ...
            sql = "select {0}, avg((st_summarystats(rast)).mean) from {1}.{2}, {1}.agareas where st_intersects(rast,geom) and gid={3} and {4} group by gid,{0} order by fdate".format(",".join(sqlvars), self.name, varnames[0], gid, date_after_nowcast_sql)
            cur.execute(sql)
            ...
            sql = "select {0}, avg((st_summarystats(rast)).mean) from {1}.{2}, {1}.agareas where st_intersects(rast,geom) and {3} group by {0} order by fdate".format(",".join(sqlvars), self.name, varnames[0], date_upto_nowcast_sql)
            cur.execute(sql)
        ...
        while not valid_point:
            ...
            if '(' in random_point: 
              random_point = random_point.split('(')[1]
            if ')' in random_point: 
              random_point = random_point.split(')')[0]
            #print(random_point)
        ...
        for varname in varnames:
            ...
            #    cur.execute(sql)
            #    results_after_nowcast = cur.fetchall()
            #    print('First: ',len(results_after_nowcast))
            #    #This is the old query where averages the whole domain weather.
            #    # It was moved here just to not mess up for the case of having results after nowcast
            ...
            #    cur.execute(sql)
            #    results_upto_nowcast = cur.fetchall()
            #    print('Second: ',len(results_after_nowcast))
    ...
    def readPlanting(self, lat, lon, fromShapefile=False):
        ...
        for yr in range(self.startyear, self.endyear + 1):
            for r in results:
                if r[0] is not None:
                    #print(date(yr,1,1,)+timedelta(r[0]-1))
        ...
        ### reverting back to same year planting - good enought for AL (SE-corn) ###
        ## Aug 2025
        ## Vikalp Mishra
        #-------------------------------
    ...
    def analysis(self):
        from rheas import result_analysis
        result_analysis.res_analysis(self,dssat=True)
