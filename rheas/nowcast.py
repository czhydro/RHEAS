...
from . import config, dbio, ensemble, raster, vic
...
#--------------------------------------------
# added function to check for vic routing 
# option in the conf file
# By: Vikalp Mishra
#     SERVIR SCO
#     Nov 2021
#-------------------------------------------- 
def _rout(vicoptions):
    """Should VIC outputs be routed?"""
    if 'rout' in vicoptions:
        rout_init = vicoptions['rout']
    else:
        rout_init = False
    if 'rout path' in vicoptions:
        routfilepath = vicoptions['rout path']
    else:
        routfilepath = ""
    return rout_init, routfilepath

#--------------------------------------------
# added function check for result analysis
# option
# UASAGE: under VIC and DSSAT 
#           Analysis <True> <False - Defualt value>
# By: Vikalp Mishra
#     UAH-ESSC
#     Aug 2025
#-------------------------------------------- 
def _analysis(options):
...
def runDeterministicVIC(dbname, options):
    ...
    model = vic.VIC(path, dbname, res, startyear, startmonth,
                    startday, endyear, endmonth, endday, basin, name)
    savestate, dbsavestate = _saveState(options['vic'])
    init, statefile = _initialize(options['vic'])
    rout_init,routfilepath = _rout(options['vic'])   #getting VIC routing config filepath 
    vic_analysis = _analysis(options['vic']) 
    ...
    #model.run(vicexe)
    # Handle parallel simulation in here
    ncores = options['vic'].get('ncores', '1')
    if ncores != '1':
        model.run_parallel(vicexe, ncores)
    else:
        model.run(vicexe)

    model.save(saveto, savevars)
    if bool(rout_init):
        model.rout(routfilepath)
    
    if bool(vic_analysis):
        model.analysis(rout_init)
    #shutil.rmtree(path)
...
def runDSSAT(dbname, options):
    ...
    dssat_analysis = _analysis(options['dssat']) 
    
    for crop in crops:
        crop = crop.strip()
        try:
            mod = __import__("rheas.dssat.{0}".format(crop), fromlist=[crop])
        except ImportError:
    ...
    # Analysis mode for DSSAT - VMishra
    # 2025
    if bool(dssat_analysis):
        model.analysis()
