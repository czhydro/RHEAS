    ...
    for section in options:
        if 'initialize' in options[section]:
            try:
                ...
                pass
        if 'rout' in options[section]:
            try:
                options[section]['rout']=conf.getboolean(section,'rout')
            except:
                pass
...
def getVICExecutable(options):
    ...
    default location if not given."""
    log = logging.getLogger(__name__)
    vicexe = _findExecutable("vicNl")
    if 'exe' in options:
        vicexe = options['exe']
    if not vicexe:
        vicexe = _findExecutable("vicNl",
                                 os.path.dirname(os.path.abspath(__file__)) + "/../external/vic/src/")
    try:
        assert vicexe is not None
    except AssertionError:
        log.error("Cannot find VIC executable! Exiting...")
        sys.exit()
    return vicexe
...
def getVICvariables(options):
    ...
    if 'save to' in options['vic']:
        ...
        if (any(v in savevars for v in ['drought', 'spi', 'cdi']) or any(v.startswith("spi") for v in savevars)) and 'rainf' not in savevars:
            savevars.insert(0, 'rainf')
        if (any(v in savevars for v in ['drought', 'severity', 'sri']) or any(v.startswith("sri") for v in savevars)) and 'runoff' not in savevars:
