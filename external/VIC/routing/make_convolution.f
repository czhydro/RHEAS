     ...
     &  MO, YR, NYR)
      ...
      CHARACTER*20 LOC, str_lat, str_lon
      CHARACTER*200 INPATH, FluxFile
...
c     find the revised cooordinates
         ILOC=XC + (ICOL-II)*SIZE + SIZE/1.0
         JLOC=YC + JJ*SIZE - SIZE/1.0
...
c	 call match_grid(INPATH,jloc,iloc, FluxFile)
	 write(str_lat, '(F10.3)') jloc
	 write(str_lon, '(F11.3)') iloc
	 print*,str_lat,str_lon
	 FluxFile = trim(adjustl(INPATH))//trim(adjustl(str_lat))//'_'//trim(adjustl(str_lon))
	 
         INQUIRE(FILE=INPATH(1:(INDEX(INPATH,' ')-1))//LOC(1:CLEN),
     $                     EXIST=TORF1)
         INQUIRE(FILE=trim(adjustl(FluxFile)),EXIST=TORF2)
         !write(*,*)INPATH,jloc,iloc, clen
         !write(FluxFile,'(A, F10.4, A, F11.4)')trim(adjustl(INPATH)),trim(jloc),'_',trim(iloc)
	 !print*, FluxFile,TORF2
     ...
     &             ' NOT FOUND, INSERTING ZEROS'
	   ...
	   do i=1,nday
         ...
         DO I = 1,NDAY
            ...
            RUNO(I) = RUNO(I) * FACTOR*0.7
            BASE(I) = BASE(I) * FACTOR*0.1
         END DO
         DO I = 1,NDAY
            DO J = 1,KE+UH_DAY-1
               IF ((I-J+1) .GE. 1) THEN
